 ...
 * Correction and modification by cliegre@gmail.com to use in codingame challenges
 * Added dijkstra algorithm support
 * Added previous mechanism to handle easy path searching
 * Added pathBeginWith method to check for common path between two Vertex
 * Correction in hasEdge
 * Change edge cost to double
 * 
...
public class Graph<T> {
	 ...
	 */
	public boolean addEdge(Vertex<T> from, Vertex<T> to, double cost) throws IllegalArgumentException {
	...
	public void dijkstra(Vertex<T> from) {
		...
		for (Vertex<T> v : getVerticies()) {
			v.setMarkState(Double.MAX_VALUE);
			v.setPrevious(null);
			if (v.equals(from)) {
				v.setMarkState(0);
			}
		...
		while ( ! finished) {
			...
			if (lowestMark != null) {
				...
				for (Edge<T> e : lowestMark.getOutgoingEdges()) {
					Vertex<T> v = e.getTo();
					if ( ! v.visited()) {
						double lastVertextAndEdgeCost = lowestMark.getMarkState() + e.getCost();
						if (v.getMarkState() > lastVertextAndEdgeCost) {
							v.setMarkState(lastVertextAndEdgeCost);
							v.setPrevious(lowestMark);
						}
					}
				}
			} else {
...
class Vertex<T> {
	...
	private double markState;

	private Vertex<T> previousName;
	...
	public void setPrevious(Vertex<T> previousName) {
	...
	public boolean pathBeginWith(Vertex<T> end) {
		boolean unfoldEnd = false;
		Vertex<T> otherprev = null;
		Vertex<T> myprev = getPrevious();
		while (myprev != null) {
			if (myprev.equals(end)) { // start of potential common path
				otherprev = end;
				unfoldEnd = true;
			}
			
			if (unfoldEnd && ! myprev.equals(otherprev)) {
				return false;
			}
			
			if (unfoldEnd) {
				otherprev = otherprev.getPrevious();
			}
			myprev = myprev.getPrevious();
		}
		if (unfoldEnd && otherprev == null) {
			return true;
		}
		return false;
	}
	...
	public boolean isLeaf() {
		if (getOutgoingEdgeCount() == 1 && getIncomingEdgeCount() == 1) {
			return outgoingEdges.get(0).getTo().equals(incomingEdges.get(0).getFrom());
		}
		return false;
	}
	 ...
	 */
	// TODO PEUT ETRE UN SOUCIS ICI CAR getFrom DEVRAIT CHECK LES outgoing
	 ...
	 *          the state
	 */
	public void setMarkState(double state) {
		markState = state;
	}
