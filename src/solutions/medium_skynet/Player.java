...
class Player {
    ...
    private static int findMostInDangerGateIndex(Vertex<String>[] vertices, int[] gates) {
        int shortest = Integer.MAX_VALUE;
        int mostInDangerGateIndex = -1;
        int E = gates.length;
        for (int i = 0; i < E; i++) {
            int costOfIthGate = vertices[gates[i]].getMarkState();
    		if (costOfIthGate < shortest) {
    		    mostInDangerGateIndex = i;
    		    shortest = costOfIthGate;
    		}
        }
        return mostInDangerGateIndex;
    }
    ...
    public static void main(String args[]) {
        ...
        Vertex<String>[] vertices = new Vertex[N];
        int[] gates = new int[E];

        // Store vertices        
        for (int i = 0; i < N; i++) {
			Vertex<String> v = new Vertex<String>(String.valueOf(i));
			vertices[i] = v;
			graph.addVertex(v);
		}
        ...
        for (int i = 0; i < L; i++) {
            ...
            int N2 = in.nextInt();
            //System.err.println("N1 N2 " + N1 + " " + N2);
            graph.insertBiEdge(vertices[N1], vertices[N2], 1);
        }
        
        // Store gates
        for (int i = 0; i < E; i++) {
            int EI = in.nextInt(); // the index of a gateway node
            //System.err.println("gate " + i + " is at node " + EI);
            gates[i] = EI;
        }
        ...
        while (true) {
            ...
            System.err.println("Agent at node " + SI);

            // shortest path to all nodes from agent position
            graph.dijkstra(vertices[SI]);
            
            // check shortest path to all gates
            int mostInDangerGateIndex = findMostInDangerGateIndex(vertices, gates);
            
            // CUT THE NODE IN FRONT OF AGENT NODE ON THE PATH TO MOST IN DANGER GATE
            String end = vertices[gates[mostInDangerGateIndex]].getName();
		    String begin = vertices[SI].getName();
		    System.err.println("most in danger gate at node " + end);

		    while(true) {
		        String previous = graph.findVertexByName(end).getPreviousName();
		        if (previous.equals(begin)) {
		            //System.err.println("Link before agent is at node " + end + " SO SEVERE");
		            break;
		        } else {
		            end = previous;
		        }
		    ...
		    graph.removeEdge(vertices[SI], vertices[Integer.parseInt(end)]);
        }
    }
}
...
class Graph<T> {
	...
	public void dijkstra(Vertex<T> from) {
		...
		for (Vertex<T> v : getVerticies()) {
			v.setMarkState(Integer.MAX_VALUE);
			if (v.equals(from)) {
				v.setMarkState(0);
			}
		}
		...
		while ( ! finished) {
			Vertex<T> lowestMark = findUnmarkedVertexWithLowestMarkState();
			if (lowestMark != null) {
				lowestMark.mark();
				for (Edge<T> e : lowestMark.getOutgoingEdges()) {
					Vertex<T> v = e.getTo();
					if ( ! v.visited()) {
						int lastVertextAndEdgeCost = lowestMark.getMarkState() + e.getCost();
						if (v.getMarkState() > lastVertextAndEdgeCost) {
							v.setMarkState(lastVertextAndEdgeCost);
							v.setPreviousName(lowestMark.getName());
						}
					}
				}
			} else {
	...
	private Vertex<T> findUnmarkedVertexWithLowestMarkState() {
		Vertex<T> lowest = null;
		for (Vertex<T> v : getVerticies()) {
			if ( ! v.visited()) {
				if (lowest == null || v.getMarkState() < lowest.getMarkState()) {
					lowest = v;
				}
			}
		}
